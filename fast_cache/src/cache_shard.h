...
namespace fast_cache
{
  ...
  public:
        ...
        : parent_(parent), evite_policy_(evite_policy)
    {
      init(capacity);
      // 开始启动异步任务
      data_merger_thread_ = std::thread([this_ptr = this]()
                                        {
      while (!this_ptr->stop_flag_.load(std::memory_order_relaxed)) {
        if (this_ptr->need_evite_) {
          spdlog::info("need merge data");
          this_ptr->backendDataMergeTasker();
          this_ptr->need_evite_.store(false);
        }
        std::this_thread::sleep_for(
            std::chrono::milliseconds(this_ptr->data_evite_interval_));
        if (!this_ptr->need_evite_) {
          spdlog::info("need evite data use data_evite_interval_:{}",
                       this_ptr->data_evite_interval_);
          this_ptr->need_evite_.store(true, std::memory_order_relaxed);
        }
      } });
    }
    // CacheShard(CacheShard &&other) noexcept = default;
    // CacheShard &operator=(CacheShard &&other) noexcept = default;
    // 禁用拷贝构造和赋值
  ...
  inline void CacheShard<K, V>::updateStatInfo(IndexEntry<K> &index_entry)
  {
    ...
    {
      spdlog::info("data_invalid_size_ > capacity_ * "
                   "data_evite_invalid_rate_threahold_");
  ...
  template <typename K, typename V>
  inline bool CacheShard<K, V>::empty()
  {
  ...
  template <typename K, typename V>
  inline int64_t CacheShard<K, V>::size()
  {
    std::shared_lock<std::shared_mutex> lock(shard_mutex_);
    return indexs_.size();
  }
  ...
  template <typename K, typename V>
  inline void CacheShard<K, V>::clear()
  {
    std::unique_lock<std::shared_mutex> lock(shard_mutex_);
    indexs_.clear();
    data_.clear();
