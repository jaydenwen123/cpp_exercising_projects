...
{
  fast_cache::CacheShard<std::string, std::string> cache(100 * 1024 * 1024, nullptr, fast_cache::LRU);
  std::string key = "test_key";
  std::string value = "test_value";

  cache.set(key, value);
  spdlog::info("SetAndDel set size:{}", cache.size());
  EXPECT_EQ(cache.size(), 1);
  // 删除键值对
  cache.del(key);
  spdlog::info("SetAndDel del size:{}", cache.size());
  EXPECT_EQ(cache.size(), 0);
